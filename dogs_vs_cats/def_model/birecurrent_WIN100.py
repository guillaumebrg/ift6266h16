...
from keras.layers.core import Dense, Flatten, Dropout, Activation, Reshape, Permute, Merge
...
def define_model(lr, momentum):
    ...
    model.add(activation())
    ### RECURRENT PART
    model.add(Reshape((256, 6*6)))
    model.add(Permute((2,1)))
    left = LSTM(64, input_length=36, return_sequences=True)
    right = LSTM(64, input_length=36, return_sequences=True, go_backwards=True)
    model.add(Merge([left, right], mode='sum'))
    model.add(LSTM(64, input_length=36))
    model.add(Dense(2))
    ...
    trainable = False
    if pretrained:
        weight_path = "pretrained_models/win100_conv.pkl"
