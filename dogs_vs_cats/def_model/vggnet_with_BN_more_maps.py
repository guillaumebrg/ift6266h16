...
def define_model(lr, momentum):
    ...
    # CONVNET
    layers = []
    layers.append(Convolution2D(16, 3, 3, activation = "linear", input_shape=(3,150,150)))
    layers.append(BatchNormalization(axis=1))
    layers.append(Activation("relu"))
    layers.append(Convolution2D(16, 3, 3, activation = "linear"))#, W_regularizer=l2(0.0001)))
    layers.append(BatchNormalization(axis=1))
    layers.append(Activation("relu"))
